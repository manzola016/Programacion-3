...
public class ContactoServicioImpl implements ContactoServicio {
    ...
    public List<ContactoEntity> obtenerContactos() {
        return contactoRepositorio.findAllWithRelations();
    }
    ...
    public void generarReporte() {
        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter("reporte.txt"))) {
            for (ContactoEntity contacto : contactos) {
                ...
                writer.newLine();
                if (contacto.getRelaciones() != null && !contacto.getRelaciones().isEmpty()) {
                    writer.write("  Empresas Relacionadas:");
                    writer.newLine();
                    for (ContactoEmpresaEntity relacion : contacto.getRelaciones()) {
                        writer.write("    - Empresa: " + relacion.getEmpresa().getNombre() + ", Direccion: " + relacion.getEmpresa().getDireccion() + ", Rol: " + relacion.getRol());
                        writer.newLine();
                    }
                } else {
                    writer.write("  Sin empresas relacionadas");
                    writer.newLine();
                }
            }
